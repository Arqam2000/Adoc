...
const ForgotPasswordDoctor = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axios.post(`${apiBaseUrl}/api/v1/doctors/request-otp`, { email })
  ...
  const handleOTPSubmit = async (e) => {
    ...
    try {
      const res = await axios.post(`${apiBaseUrl}/api/v1/doctors/verify-otp`, { email, otp })

      if (res.data.success) {
        toast.success(res.data.message)
        setIsOtpSent(false)
        navigate('/reset-password/doctor', { state: { email } })
      } else {
        toast.error(res.data.message)
      }
    } catch (error) {
