...
const ResetPasswordDoctor = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true)
    try {
      const res = await axios.post(`${apiBaseUrl}/api/v1/doctors/reset-password`, { email: location?.state?.email, password, confirmPassword })
  
      if (res.data.success) {
        toast.success(res.data.message)
        localStorage.setItem("doctorId", JSON.stringify(res.data.doctor.dr))
        setLoginName(res.data.doctor.name)
        setPd("d")
        navigate('/dashboard')
      }
    } catch (error) {
